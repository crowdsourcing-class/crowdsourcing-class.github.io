...
* [Crowds in Two Seconds: Enabling Realtime Crowd-Powered Interfaces](readings/downloads/hci/adrenaline.pdf) by Michael Bernstein, Joel Brandt, Rob Miller, and David Karger - Please note you only need to replicate the alert experiment of the paper but not the real-time app
...
Create a task on Mechanical Turk or another crowdsourcing platform that you'll use to perform the experiment.  You should write clear instructions on what you'd like the workers to do (and how they will be rewarded, if that's relevant to the experiment).
